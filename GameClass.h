...
class BaseWeapon {
public:
	int qtyOfDamage;// олличество кубов
	int damageBonus;//Ѕонус урона
	int hitBonus;//Ѕонус попадани€ 
	int numberOfAttacks;
};
...
public:
	...
	int howMuchDamage();//¬озвращает значени€ полученного урона
...
inline int BaseCharacter::attack()
{
	...
	if (rollHit < rollRange(10,11)) return 0; // если не попал то урон 0, дистанци€ рандомна€ от 10 до 20 
	...
	case 1:
		...
		break;
	case 2:
		switch ((int)std::floor(rollDice(4) / 2))//округл€ем до int колличество попаданий
		{
		case 1:
			totalDamage += dealWeaponDamage(weapon.qtyOfDamage, 1);
			break;
		...
		break;
	case 3:
		switch ((int)std::floor(rollDice(6) / 2))//округл€ем до int колличество попаданий
		{
		case 1:
			totalDamage += dealWeaponDamage(weapon.qtyOfDamage, 1);
			break;
	...
	if (rollDice(100) == 100) totalDamage += rollDice(6) * 7; // ѕроцентный шанс того что чел еще бросит гранату
